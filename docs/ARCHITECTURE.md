...
| Path | Purpose | Status |
| --- | --- | --- |
| `apps/mobile` | Expo app. UI only - no core business logic. | Phase 0 scaffold |
...
| `packages/exercise-catalog` | Canonical exercise catalog pipeline. | Phase 2 |
...
| `scripts` | Fixture generation, license check. | Phase 0/1 |
| `datasets` | Pinned upstream datasets + `sources.lock.json`. | Phase 2 |
...
- **TypeScript:** strict + `noUncheckedIndexedAccess` + `verbatimModuleSyntax`
  (see `tsconfig.base.json`). TS 5.9 pinned for toolchain compatibility
  (typescript-eslint supports < 6.1).
- **Expo SDK 57** (React Native 0.87, React 19.2), Expo Router tab navigation.
